...
export class VoiceCell extends MessageCell<any, VoiceCellState> {
  ...
  constructor(props: any) {
    ...
    if (this.content.waveform && this.content.waveform.length > 0) {
      this.waveform = new Uint8Array(
        atob(this.content.waveform)
          .split("")
  ...
  playOrPauseVoice(e: any) {
    ...
    const voiceURL = WKApp.dataSource.commonDataSource.getFileURL(
      this.content.url
    );
    ...
    voicePlayer.onPlay(() => {
      ...
      this.timer = setInterval(() => {
        const progress =
          (voicePlayer.getCurrentPosition() / voicePlayer.getDuration()) * 100;
        // console.log(progress)
        if (this.lightWavformRef.current) {
          this.lightWavformRef.current.style.width = `${progress}%`;
        }
        if (this.timeRef.current) {
          this.timeRef.current.innerText = this.formatSecond(
            voicePlayer.getDuration() - voicePlayer.getCurrentPosition()
          );
        }
        // this.setState({
  ...
  render() {
    ...
    return (
      <MessageBase message={message} context={context}>
        ...
        <div className="yw-message-voice">
          <div
            className={classNames("voicePlay", this.getPlayStatusClassname())}
            onClick={(e) => {
              ...
              <div className="progressSpinner">
                <svg viewBox="0 0 48 48" height="48" width="48">
                  <circle
                    ...
                    fill="transparent"
                    strokeWidth="2"
                    strokeDasharray="131.94689145077132 131.94689145077132"
                    strokeDashoffset="125.34954687823276"
                    strokeLinecap="round"
                    r="21"
                    cx="24"
                    cy="24"
          ...
          <div className="yw-message-voice-info">
            <div className="yw-message-voice-waveform">
              <WaveCanvas
                barColor={
                  isSend ? "rgb(255, 255, 255,0.5)" : "rgb(0, 0, 0,0.2)"
                }
              ...
              />
              <div
                ref={this.lightWavformRef}
                className="yw-message-voice-lightWavform"
              >
                <WaveCanvas
                  barColor={isSend ? "#fff" : WKApp.config.themeColor}
                  waveform={this.waveform ?? []}
                  width={200}
                  height={23}
                />
            ...
            <div className="yw-message-voice-info-status">
              <div className="yw-message-voice-info-time" ref={this.timeRef}>
                {this.timeFormat}
                {/* <div className={style.status}>*</div> */}
              </div>
              <div className="yw-message-voice-info-tail">
