...
export class ChatVM extends ProviderListener {
  ...
  private conversationListID = "yw-conversationlist";
  ...
  didMount(): void {
    ...
    this.connectStatusListener = (
      ...
      this.setConnectTitleWithConnectStatus(
        WKSDK.shared().connectManager.status
      );
      ...
      if (action === ConversationAction.add) {
        ...
        if (
          ...
          conversation.lastMessage.content.text =
            ProhibitwordsService.shared.filter(
              conversation.lastMessage?.content.text
            );
        }
        this.conversations = [
        ...
        if (existConversation) {
          ...
          if (
            existConversation.lastMessage?.content &&
            existConversation.lastMessage?.contentType ===
              MessageContentType.text
          ) {
            existConversation.lastMessage.content.text =
              ProhibitwordsService.shared.filter(
                existConversation.lastMessage?.content.text
              );
          }
    ...
    this.messageDeleteListener = (message: Message, preMessage?: Message) => {
      ...
      if (conversation) {
        if (
          ...
          conversation.lastMessage.clientMsgNo === message.clientMsgNo
        ) {
